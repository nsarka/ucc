...
#include "../kernels/allreduce_kernel.h" // nick TODO: change to allgatherv_kernel.h?
...
ucc_status_t ucc_tl_cuda_allgatherv_nvls_start(ucc_coll_task_t *coll_task)
{
    ...
    task->allgatherv_nvls.buf_size_bytes = args->dst.info.count * ucc_dt_size(task->allgatherv_nvls.dt);
    task->allgatherv_nvls.rbuf           = args->dst.info.buffer;
    task->allgatherv_nvls.sbuf           = UCC_IS_INPLACE(*args) ? args->dst.info.buffer : args->src.info.buffer;

    tl_trace(UCC_TASK_LIB(task),
             "task: %p stream: %p allgatherv_nvls_start symmetric uc addr: %p "
             "mc addr: %p "
             "buf_size_bytes: %zu, is inplace: %d",
             task, stream, (void *)task->allgatherv_nvls.uc_va, (void *)task->allgatherv_nvls.mc_va,
             task->allgatherv_nvls.buf_size_bytes, UCC_IS_INPLACE(*args));
...
void ucc_tl_cuda_allgatherv_nvls_progress(ucc_coll_task_t *coll_task)
{
    ...
    ucc_ec_cuda_event_t *ec_event = (ucc_ec_cuda_event_t *)task->allgatherv_nvls.evtCompletion;
    ...
    case STAGE_KERNEL:
        ...
        cuda_status =
            cudaMemcpyAsync((void *)uc_va, task->allgatherv_nvls.sbuf,
                            task->allgatherv_nvls.buf_size_bytes,
        ...
        cuda_status = cudaMemcpyAsync((void *)task->allgatherv_nvls.rbuf,
                        (void *)uc_va,
                        task->allgatherv_nvls.buf_size_bytes,
        ...
        if (cuda_status != cudaSuccess) {
            ucc_error("task: %p, cudaMemcpyAsync failed: %s, stream: %p, sbuf: "
                      ...
                      task->allgatherv_nvls.sbuf, task->allgatherv_nvls.rbuf,
                      (void *)uc_va, task->allgatherv_nvls.buf_size_bytes);
...
ucc_status_t ucc_tl_cuda_allgatherv_nvls_finalize(ucc_coll_task_t *task)
{
    ...
    ucc_ec_destroy_event(tl_task->allgatherv_nvls.evtCompletion, UCC_EE_CUDA_STREAM);
...
ucc_status_t ucc_tl_cuda_allgatherv_nvls_triggered_post(ucc_ee_h ee, ucc_ev_t *ev,
...
ucc_status_t ucc_tl_cuda_allgatherv_nvls_init(ucc_base_coll_args_t *coll_args,
                                             ...
                                             ucc_coll_task_t     **task_p)
{
    ...
    if (buf_size < 1024 ||
        ...
        tl_debug(UCC_TL_TEAM_LIB(team), "NVLS allgatherv is supported only with SUM operation "
    ...
    status = ucc_ec_create_event(&task->allgatherv_nvls.evtCompletion, UCC_EE_CUDA_STREAM);
    if (ucc_unlikely(status != UCC_OK)) {
    ...
    task->allgatherv_nvls.dt = coll_args->args.dst.info.datatype;
    tl_trace(UCC_TL_TEAM_LIB(team), "NVLS allgatherv datatype: %s", ucc_datatype_str(task->allgatherv_nvls.dt));
