...
static int ucc_tl_ucp_alltoallv_nsarka_register(
    ucp_context_h ucp_context, ucc_tl_ucp_team_t *tl_team,
    struct ucc_tl_ucp_alltoallv_nsarka_export_buf *ebuf, void *packed_memh)
{
                                              ...
                                              ucc_tl_ucp_task_t *   task)
{
    ...
    void *                src_buf   = coll_args->args.src.info_v.buffer;
    ...
    ucc_tl_ucp_alltoallv_nsarka_global_work_buf_info *gwbi_p = NULL;
    ...
    task->alltoallv_nsarka.inplace = UCC_IS_INPLACE(coll_args->args);

    task->alltoallv_nsarka.barrier_task = NULL;

    if (!task->alltoallv_nsarka.inplace) {
        task->alltoallv_nsarka.sbufs =
            ucc_malloc(sizeof(void *) * team_size);
        task->alltoallv_nsarka.src_rkeys =
            ucc_malloc(sizeof(ucp_rkey_h) * team_size);
    }

    task->alltoallv_nsarka.rbufs =
        ucc_malloc(sizeof(void *) * team_size);
    task->alltoallv_nsarka.dst_rkeys =
        ucc_malloc(sizeof(ucp_rkey_h) * team_size);
    task->alltoallv_nsarka.eps =
        ucc_malloc(sizeof(ucp_ep_h) * team_size);


    if (!task->alltoallv_nsarka.inplace) {
        task->alltoallv_nsarka.src_ebuf =
            ucc_malloc(sizeof(struct ucc_tl_ucp_alltoallv_nsarka_export_buf));
    } else {
        task->alltoallv_nsarka.src_ebuf = NULL;
    }

    task->alltoallv_nsarka.dst_ebuf =
        ucc_malloc(sizeof(struct ucc_tl_ucp_alltoallv_nsarka_export_buf));
    ...
    ucc_tl_ucp_alltoallv_nsarka_register(
        tl_ctx->worker.ucp_context, tl_team,
        task->alltoallv_nsarka.dst_ebuf, gwbi_p->packed_dst_memh);
    memcpy(allgather_data->packed_dst_key,
           task->alltoallv_nsarka.dst_ebuf->packed_key,
           task->alltoallv_nsarka.dst_ebuf->packed_key_len);
    
    if (!task->alltoallv_nsarka.inplace) {
        ucc_tl_ucp_alltoallv_nsarka_register(
            tl_ctx->worker.ucp_context, tl_team,
            task->alltoallv_nsarka.src_ebuf, gwbi_p->packed_src_memh);
        memcpy(allgather_data->packed_src_key,
               task->alltoallv_nsarka.src_ebuf->packed_key,
               task->alltoallv_nsarka.src_ebuf->packed_key_len);
    }

    task->alltoallv_nsarka.allgather_data      = allgather_data;
    ...
    ucc_service_coll_req_t *scoll_req, ucc_tl_ucp_task_t *task)
{
    ...
    size_t allgather_size = sizeof(ucc_tl_ucp_alltoallv_nsarka_host_allgather);
    ucc_tl_ucp_alltoallv_nsarka_host_allgather *all_host_allgather =
    ...
    for (i = 0; i < team_size; i++) {
        ...
        task->alltoallv_nsarka.rbufs[i] =
            all_host_allgather[i].dst_buf;
        task->alltoallv_nsarka.dst_rkeys[i] = dst_unpacked;
        ...
        if (!task->alltoallv_nsarka.inplace) {
            ...
            task->alltoallv_nsarka.sbufs[i] =
                all_host_allgather[i].src_buf;
            task->alltoallv_nsarka.src_rkeys[i] = src_unpacked;
        } else {
            task->alltoallv_nsarka.sbufs =
                task->alltoallv_nsarka.rbufs;
            task->alltoallv_nsarka.src_rkeys =
        ...
        task->alltoallv_nsarka.eps[i] = ep;
    }
...
ucc_status_t
ucc_tl_ucp_alltoallv_nsarka_free_gwbi(ucc_coll_task_t *coll_task)
{
    ...
    if (!task->alltoallv_nsarka.inplace)
        ucc_free(task->alltoallv_nsarka.sbufs);

    ucc_free(task->alltoallv_nsarka.rbufs);
    ucc_free(task->alltoallv_nsarka.eps);
    ucc_free(task->alltoallv_nsarka.allgather_data);

    if (!task->alltoallv_nsarka.inplace) {
        ucp_mem_unmap(tl_ctx->worker.ucp_context,
                      task->alltoallv_nsarka.src_ebuf->memh);
        ucc_free(task->alltoallv_nsarka.src_ebuf);
        ucc_free(task->alltoallv_nsarka.src_rkeys);
    }

    ucp_mem_unmap(tl_ctx->worker.ucp_context,
                  task->alltoallv_nsarka.dst_ebuf->memh);
    ucc_free(task->alltoallv_nsarka.dst_ebuf);
    ucc_free(task->alltoallv_nsarka.dst_rkeys);
