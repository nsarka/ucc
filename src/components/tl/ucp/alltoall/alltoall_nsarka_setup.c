...
static int ucc_tl_ucp_alltoall_nsarka_register(
    ucp_context_h ucp_context, ucc_tl_ucp_team_t *tl_team,
    struct ucc_tl_ucp_alltoall_nsarka_export_buf *ebuf, void *packed_memh)
{
                                              ...
                                              ucc_tl_ucp_task_t *   task)
{
    ...
    ucc_tl_ucp_alltoall_nsarka_global_work_buf_info *gwbi_p = NULL;
    ...
    task->alltoall_nsarka.inplace = UCC_IS_INPLACE(coll_args->args);

    task->alltoall_nsarka.barrier_task = NULL;

    if (!task->alltoall_nsarka.inplace) {
        task->alltoall_nsarka.sbufs =
            ucc_malloc(sizeof(void *) * team_size);
        task->alltoall_nsarka.src_rkeys =
            ucc_malloc(sizeof(ucp_rkey_h) * team_size);
    }

    task->alltoall_nsarka.rbufs =
        ucc_malloc(sizeof(void *) * team_size);
    task->alltoall_nsarka.dst_rkeys =
        ucc_malloc(sizeof(ucp_rkey_h) * team_size);
    task->alltoall_nsarka.eps =
        ucc_malloc(sizeof(ucp_ep_h) * team_size);


    if (!task->alltoall_nsarka.inplace) {
        task->alltoall_nsarka.src_ebuf =
            ucc_malloc(sizeof(struct ucc_tl_ucp_alltoall_nsarka_export_buf));
    } else {
        task->alltoall_nsarka.src_ebuf = NULL;
    }

    task->alltoall_nsarka.dst_ebuf =
        ucc_malloc(sizeof(struct ucc_tl_ucp_alltoall_nsarka_export_buf));
    ...
    if (!task->alltoall_nsarka.inplace) {
        ucc_tl_ucp_alltoall_nsarka_register(
            tl_ctx->worker.ucp_context, tl_team,
            task->alltoall_nsarka.src_ebuf, gwbi_p->packed_src_memh);
        memcpy(allgather_data->packed_src_key,
               task->alltoall_nsarka.src_ebuf->packed_key,
               task->alltoall_nsarka.src_ebuf->packed_key_len);
    }

    ucc_tl_ucp_alltoall_nsarka_register(
        tl_ctx->worker.ucp_context, tl_team,
        task->alltoall_nsarka.dst_ebuf, gwbi_p->packed_dst_memh);
    memcpy(allgather_data->packed_dst_key,
           task->alltoall_nsarka.dst_ebuf->packed_key,
           task->alltoall_nsarka.dst_ebuf->packed_key_len);

    task->alltoall_nsarka.allgather_data      = allgather_data;
    ...
    ucc_service_coll_req_t *scoll_req, ucc_tl_ucp_task_t *task)
{
    ...
    size_t allgather_size = sizeof(ucc_tl_ucp_alltoall_nsarka_host_allgather);
    ucc_tl_ucp_alltoall_nsarka_host_allgather *all_host_allgather =
    ...
    for (i = 0; i < team_size; i++) {
        ...
        task->alltoall_nsarka.rbufs[i] =
            all_host_allgather[i].dst_buf;
        task->alltoall_nsarka.dst_rkeys[i] = dst_unpacked;
        ...
        if (!task->alltoall_nsarka.inplace) {
            ...
            task->alltoall_nsarka.sbufs[i] =
                all_host_allgather[i].src_buf;
            task->alltoall_nsarka.src_rkeys[i] = src_unpacked;
        } else {
            task->alltoall_nsarka.sbufs =
                task->alltoall_nsarka.rbufs;
            task->alltoall_nsarka.src_rkeys =
...
ucc_status_t
ucc_tl_ucp_alltoall_nsarka_free_gwbi(ucc_coll_task_t *coll_task)
{
    ...
    if (!task->alltoall_nsarka.inplace)
        ucc_free(task->alltoall_nsarka.sbufs);

    ucc_free(task->alltoall_nsarka.rbufs);
    ucc_free(task->alltoall_nsarka.eps);
    ucc_free(task->alltoall_nsarka.allgather_data);

    if (!task->alltoall_nsarka.inplace) {
        ucp_mem_unmap(tl_ctx->worker.ucp_context,
                      task->alltoall_nsarka.src_ebuf->memh);
        ucc_free(task->alltoall_nsarka.src_ebuf);
        ucc_free(task->alltoall_nsarka.src_rkeys);
    }

    ucp_mem_unmap(tl_ctx->worker.ucp_context,
                  task->alltoall_nsarka.dst_ebuf->memh);
    ucc_free(task->alltoall_nsarka.dst_ebuf);
    ucc_free(task->alltoall_nsarka.dst_rkeys);
