...
ucc_base_coll_alg_info_t
    ucc_tl_ucp_alltoall_algs[UCC_TL_UCP_ALLTOALL_ALG_LAST + 1] = {
        ...
        [UCC_TL_UCP_ALLTOALL_ALG_NSARKA] =
            {.id   = UCC_TL_UCP_ALLTOALL_ALG_NSARKA,
             .name = "nsarka",
             .desc = "nsarka alltoall (optimized for running on DPU)"},
...
ucc_status_t
ucc_tl_ucp_alltoall_nsarka_init(ucc_base_coll_args_t *coll_args,
                                         ucc_base_team_t *     team,
                                         ucc_coll_task_t **    task_h)
{
    ...
    *task_h              = &task->super;
    task->super.post     = ucc_tl_ucp_alltoall_nsarka_start;
    task->super.progress = ucc_tl_ucp_alltoall_nsarka_progress;
    task->super.finalize = ucc_tl_ucp_alltoall_nsarka_finalize;

    ucc_tl_ucp_alltoall_nsarka_task_init(coll_args, team, task);
