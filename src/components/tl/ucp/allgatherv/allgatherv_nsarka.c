...
ucc_tl_ucp_allgatherv_nsarka_start(ucc_coll_task_t *coll_task)
{
    ...
    ucc_tl_ucp_allgatherv_nsarka_host_allgather *allgather_data =
        task->allgatherv_nsarka.allgather_data;
    ...
    task->allgatherv_nsarka.gets_posted    = 0;
    task->allgatherv_nsarka.gets_completed = 0;

    task->allgatherv_nsarka.requests = ucc_malloc(sizeof(ucs_status_ptr_t) * size);

    for(i = 0; i < size; i++) {
        task->allgatherv_nsarka.requests[i] = NULL;
    }
    ...
    task->allgatherv_nsarka.allgather_scoll_req = scoll_req;
...
ucc_status_t
ucc_tl_ucp_allgatherv_nsarka_finalize(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_status_t       st = UCC_OK;

    ucc_free(task->allgatherv_nsarka.requests);
...
ucc_tl_ucp_allgatherv_nsarka_req_test(ucs_status_ptr_t   request,
...
static inline void ucc_tl_ucp_allgatherv_nsarka_allgather_info_test(
    ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *     task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_service_coll_req_t *allgather_scoll_req =
        task->allgatherv_nsarka.allgather_scoll_req;
    ...
    ucc_tl_ucp_allgatherv_nsarka_allgather_info_finalize(
...
static inline void ucc_tl_ucp_allgatherv_nsarka_allgather_free_rkeys(
    ucc_coll_task_t *coll_task)
{
    int                i;
    ...
    for (i = 0; i < team_size; i++) {
        if (!task->allgatherv_nsarka.inplace)
            ucp_rkey_destroy(task->allgatherv_nsarka.src_rkeys[i]);
        ucp_rkey_destroy(task->allgatherv_nsarka.dst_rkeys[i]);
    }
}

static inline void
ucc_tl_ucp_allgatherv_nsarka_barrier(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_base_team_t *  team = coll_task->team;
    ucc_status_t       status;
    ...
    status = ucc_tl_ucp_coll_init(&coll_args, team,
                                  &task->allgatherv_nsarka.barrier_task);
    if (status < 0) {
        tl_error(coll_task->team->context->lib,
...
void ucc_tl_ucp_allgatherv_nsarka_progress(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task    = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_rank_t         size    = (ucc_rank_t)task->subset.map.ep_num;
    ...
    ucc_service_coll_req_t *     allgather_scoll_req =
        task->allgatherv_nsarka.allgather_scoll_req;
    ucc_coll_task_t *barrier_task = task->allgatherv_nsarka.barrier_task;
    ...
    ucs_status_ptr_t *requests = task->allgatherv_nsarka.requests;
    int *posted = &task->allgatherv_nsarka.gets_posted;
    int *completed = &task->allgatherv_nsarka.gets_completed;

    if (barrier_task != NULL) {
        // mark sliding window task complete once barrier finishes
        if (barrier_task->super.status == UCC_OK) {
            ucc_tl_ucp_put_task(
                ucc_derived_of(task->allgatherv_nsarka.barrier_task,
                               ucc_tl_ucp_task_t));
            task->allgatherv_nsarka.barrier_task = NULL;
    ...
    if (allgather_scoll_req != NULL) {
        ucc_tl_ucp_allgatherv_nsarka_allgather_info_test(coll_task);
        return;
    }
    ...
    for (i = *posted; i < host_team_size; i++) {
        ...
        req_param.memh = task->allgatherv_nsarka.dst_ebuf->memh;
        src_addr = task->allgatherv_nsarka.sbufs[i];
        dst_addr = dst_displacements[i] + task->allgatherv_nsarka.rbufs[rank];
        rkey = task->allgatherv_nsarka.src_rkeys[i];
        ep = task->allgatherv_nsarka.eps[i];
    ...
    for (i = *completed; i < *posted; i++) {
        if (ucc_tl_ucp_allgatherv_nsarka_req_test(requests[i], task) == UCC_OK) {
