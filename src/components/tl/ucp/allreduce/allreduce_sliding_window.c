...
ucc_tl_ucp_allreduce_sliding_window_start(ucc_coll_task_t *coll_task)
{
    ...
    UCC_CHECK_GOTO(ucc_tl_ucp_allgather_knomial_init(&bargs,
                    &team->super.super,
                    &task->allreduce_sliding_window.allgather_task),
        out, status);

    task->allreduce_sliding_window.allgather_task->executor =
                                        task->allreduce_sliding_window.executor;

    UCC_CHECK_GOTO(ucc_tl_ucp_allgather_knomial_start(
                    task->allreduce_sliding_window.allgather_task),
        out, status);
...
out:
    ...
    tl_error(UCC_TASK_LIB(task), "failed to start allreduce sliding window");
    return status;
}
...
ucc_tl_ucp_allreduce_sliding_window_finalize(ucc_coll_task_t *coll_task)
{
    ...
    if (ucc_unlikely(ucc_tl_ucp_allreduce_sliding_window_free_gwbi(coll_task)
            != UCC_OK)) {
        tl_error(UCC_TASK_LIB(task), "failed to free resources");
    }
    ...
    ucc_coll_task_t *coll_task)
{
    ...
    if (status < 0) {
        tl_error(coll_task->team->context->lib,
                 "failure during key exchange: %s",
                 ucc_status_string(status));
        goto out;
    }
    if (UCC_INPROGRESS == status) {
        ucc_tl_ucp_allgather_knomial_progress(allgather_task);
        return;
    }
