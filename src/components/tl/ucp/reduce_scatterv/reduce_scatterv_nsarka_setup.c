...
static int ucc_tl_ucp_reduce_scatterv_nsarka_register(
    ucp_context_h ucp_context, ucc_tl_ucp_team_t *tl_team,
    struct ucc_tl_ucp_reduce_scatterv_nsarka_export_buf *ebuf, void *packed_memh)
{
                                               ...
                                               ucc_tl_ucp_task_t *   task)
{
    ...
    ucc_tl_ucp_reduce_scatterv_nsarka_pipeline *pipe =
        (ucc_tl_ucp_reduce_scatterv_nsarka_pipeline *)ucc_malloc(
            sizeof(ucc_tl_ucp_reduce_scatterv_nsarka_pipeline));
    if (pipe == NULL) {
    ...
    pipe->getbuf = (ucc_tl_ucp_reduce_scatterv_nsarka_buf *)ucc_malloc(
        num_get_bufs * sizeof(ucc_tl_ucp_reduce_scatterv_nsarka_buf));
    if (pipe->getbuf == NULL) {
    ...
    task->reduce_scatterv_nsarka.put_window_size = put_window_size;
    task->reduce_scatterv_nsarka.num_get_bufs    = num_get_bufs;

    task->reduce_scatterv_nsarka.pipe = pipe;
                                              ...
                                              ucc_tl_ucp_task_t *   task)
{
    ...
    ucc_tl_ucp_reduce_scatterv_nsarka_global_work_buf_info *gwbi_p = NULL;
    ...
    if ((status = ucc_tl_ucp_reduce_scatterv_nsarka_alloc_pipe(
    ...
    task->reduce_scatterv_nsarka.inplace = UCC_IS_INPLACE(coll_args->args);

    task->reduce_scatterv_nsarka.barrier_task = NULL;

    if (!task->reduce_scatterv_nsarka.inplace) {
        task->reduce_scatterv_nsarka.sbufs =
            ucc_malloc(sizeof(void *) * team_size);
        task->reduce_scatterv_nsarka.src_rkeys =
            ucc_malloc(sizeof(ucp_rkey_h) * team_size);
    }

    task->reduce_scatterv_nsarka.rbufs =
        ucc_malloc(sizeof(void *) * team_size);
    task->reduce_scatterv_nsarka.dst_rkeys =
        ucc_malloc(sizeof(ucp_rkey_h) * team_size);
    task->reduce_scatterv_nsarka.eps =
        ucc_malloc(sizeof(ucp_ep_h) * team_size);

    task->reduce_scatterv_nsarka.put_requests =
        task->reduce_scatterv_nsarka.pipe->put_requests;

    if (!task->reduce_scatterv_nsarka.inplace) {
        task->reduce_scatterv_nsarka.src_ebuf =
            ucc_malloc(sizeof(struct ucc_tl_ucp_reduce_scatterv_nsarka_export_buf));
    } else {
        task->reduce_scatterv_nsarka.src_ebuf = NULL;
    }

    task->reduce_scatterv_nsarka.dst_ebuf =
        ucc_malloc(sizeof(struct ucc_tl_ucp_reduce_scatterv_nsarka_export_buf));
    ...
    if (!task->reduce_scatterv_nsarka.inplace) {
        ucc_tl_ucp_reduce_scatterv_nsarka_register(
            tl_ctx->worker.ucp_context, tl_team,
            task->reduce_scatterv_nsarka.src_ebuf, gwbi_p->packed_src_memh);
        memcpy(allgather_data->packed_src_key,
               task->reduce_scatterv_nsarka.src_ebuf->packed_key,
               task->reduce_scatterv_nsarka.src_ebuf->packed_key_len);
    }

    ucc_tl_ucp_reduce_scatterv_nsarka_register(
        tl_ctx->worker.ucp_context, tl_team,
        task->reduce_scatterv_nsarka.dst_ebuf, gwbi_p->packed_dst_memh);
    memcpy(allgather_data->packed_dst_key,
           task->reduce_scatterv_nsarka.dst_ebuf->packed_key,
           task->reduce_scatterv_nsarka.dst_ebuf->packed_key_len);

    task->reduce_scatterv_nsarka.allgather_data      = allgather_data;
    ...
    ucc_service_coll_req_t *scoll_req, ucc_tl_ucp_task_t *task)
{
    ...
    size_t allgather_size = sizeof(ucc_tl_ucp_reduce_scatterv_nsarka_host_allgather);
    ucc_tl_ucp_reduce_scatterv_nsarka_host_allgather *all_host_allgather =
    ...
    for (i = 0; i < team_size; i++) {
        ...
        task->reduce_scatterv_nsarka.rbufs[i] =
            all_host_allgather[i].dst_buf;
        task->reduce_scatterv_nsarka.dst_rkeys[i] = dst_unpacked;
        ...
        if (!task->reduce_scatterv_nsarka.inplace) {
            ...
            task->reduce_scatterv_nsarka.sbufs[i] =
                all_host_allgather[i].src_buf;
            task->reduce_scatterv_nsarka.src_rkeys[i] = src_unpacked;
        } else {
            task->reduce_scatterv_nsarka.sbufs =
                task->reduce_scatterv_nsarka.rbufs;
            task->reduce_scatterv_nsarka.src_rkeys =
        ...
        task->reduce_scatterv_nsarka.eps[i] = ep;
    }
...
ucc_status_t
ucc_tl_ucp_reduce_scatterv_nsarka_free_gwbi(ucc_coll_task_t *coll_task)
{
    ...
    if (!task->reduce_scatterv_nsarka.inplace)
        ucc_free(task->reduce_scatterv_nsarka.sbufs);

    ucc_free(task->reduce_scatterv_nsarka.rbufs);
    ucc_free(task->reduce_scatterv_nsarka.eps);
    ucc_free(task->reduce_scatterv_nsarka.allgather_data);

    if (!task->reduce_scatterv_nsarka.inplace) {
        ucp_mem_unmap(tl_ctx->worker.ucp_context,
                      task->reduce_scatterv_nsarka.src_ebuf->memh);
        ucc_free(task->reduce_scatterv_nsarka.src_ebuf);
        ucc_free(task->reduce_scatterv_nsarka.src_rkeys);
    }

    ucp_mem_unmap(tl_ctx->worker.ucp_context,
                  task->reduce_scatterv_nsarka.dst_ebuf->memh);
    ucc_free(task->reduce_scatterv_nsarka.dst_ebuf);
    ucc_free(task->reduce_scatterv_nsarka.dst_rkeys);
