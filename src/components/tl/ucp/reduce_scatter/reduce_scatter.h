...
ucc_status_t ucc_tl_ucp_reduce_scatter_nsarka_init(ucc_base_coll_args_t *coll_args,
                                               ucc_base_team_t *team,
                                               ucc_coll_task_t **task_h);
...
void ucc_tl_ucp_reduce_scatter_nsarka_progress(ucc_coll_task_t *task);

ucc_status_t ucc_tl_ucp_reduce_scatter_nsarka_finalize(ucc_coll_task_t *task);
...
typedef struct ucc_tl_ucp_reduce_scatter_nsarka_global_work_buf_info {
    void *packed_src_memh;
    void *packed_dst_memh;
} ucc_tl_ucp_reduce_scatter_nsarka_global_work_buf_info;

struct ucc_tl_ucp_reduce_scatter_nsarka_export_buf {
...
typedef struct ucc_tl_ucp_reduce_scatter_nsarka_buf {
    void *                            buf;
    ucc_tl_ucp_allreduce_sw_buf_state state;
    ucs_status_ptr_t                  ucp_req;
    size_t                            count;
    size_t                            bytes;
} ucc_tl_ucp_reduce_scatter_nsarka_buf;

typedef struct ucc_tl_ucp_reduce_scatter_nsarka_pipeline {
    ucc_tl_ucp_reduce_scatter_nsarka_buf  accbuf;
