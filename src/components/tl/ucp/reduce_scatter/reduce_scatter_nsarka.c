...
static inline void ucc_tl_ucp_reduce_scatter_nsarka_allgather_free_rkeys(
    ucc_coll_task_t *coll_task)
{
    ...
    for (i = 0; i < team_size; i++) {
        if (!task->reduce_scatter_nsarka.inplace)
            ucp_rkey_destroy(task->reduce_scatter_nsarka.src_rkeys[i]);
        ucp_rkey_destroy(task->reduce_scatter_nsarka.dst_rkeys[i]);
    }
}

static inline void
ucc_tl_ucp_reduce_scatter_nsarka_test_reduction(ucc_tl_ucp_task_t *task)
{
    ucc_status_t status;

#define SAVE_STATE(_reduce_in_progress) _reduce_in_progress = 1

    EXEC_TASK_TEST(task->reduce_scatter_nsarka.reduce_in_progress,
                   "failed to perform dt reduction",
                   task->reduce_scatter_nsarka.etask);

    // If it didn't complete, we would have returned by now. So, clear the flag
    task->reduce_scatter_nsarka.reduce_in_progress = 0;
}

static inline void ucc_tl_ucp_reduce_scatter_nsarka_reduction(
    ucc_coll_task_t *coll_task, ucc_tl_ucp_reduce_scatter_nsarka_buf *accbuf,
    ucc_tl_ucp_reduce_scatter_nsarka_buf *getbuf)
{
    ...
    status =
        ucc_dt_reduce(accbuf->buf, getbuf->buf, accbuf->buf, accbuf->count, dt,
                      args, 0, 0, task->reduce_scatter_nsarka.executor,
                      &task->reduce_scatter_nsarka.etask);

    task->reduce_scatter_nsarka.reduce_in_progress = 1;
...
static inline void
ucc_tl_ucp_reduce_scatter_nsarka_reset_buf(ucc_tl_ucp_reduce_scatter_nsarka_buf *buf)
{
    buf->state   = FREE;
    buf->count   = 0;
    buf->bytes   = 0;
...
static inline void ucc_tl_ucp_reduce_scatter_nsarka_reset_pipeline(
    ucc_tl_ucp_reduce_scatter_nsarka_pipeline *pipe, ucc_rank_t rank,
    size_t put_window_size)
{
    ...
    ucc_tl_ucp_reduce_scatter_nsarka_reset_buf(&pipe->accbuf);
    for (i = 0; i < pipe->num_buffers; i++) {
        ucc_tl_ucp_reduce_scatter_nsarka_reset_buf(&pipe->getbuf[i]);
    }
...
ucc_status_t
ucc_tl_ucp_reduce_scatter_nsarka_start(ucc_coll_task_t *coll_task)
{
    ucc_status_t       status = UCC_OK;
    ucc_tl_ucp_task_t *task   = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ...
    ucc_tl_ucp_reduce_scatter_nsarka_pipeline *pipe =
        task->reduce_scatter_nsarka.pipe;
    ucc_tl_ucp_reduce_scatter_nsarka_host_allgather *allgather_data =
        task->reduce_scatter_nsarka.allgather_data;
    size_t allgather_size = sizeof(ucc_tl_ucp_reduce_scatter_nsarka_host_allgather);
    ucc_service_coll_req_t *scoll_req;

    ucc_tl_ucp_reduce_scatter_nsarka_reset_pipeline(
        pipe, rank, task->reduce_scatter_nsarka.put_window_size);
    ...
    task->reduce_scatter_nsarka.allgather_scoll_req = scoll_req;
...
ucc_status_t
ucc_tl_ucp_reduce_scatter_nsarka_finalize(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_status_t       st =
        ucc_ee_executor_finalize(task->reduce_scatter_nsarka.executor);

    if (ucc_unlikely(st != UCC_OK)) {
        tl_error(UCC_TASK_LIB(task), "failed to finalize executor");
    }

    ucc_tl_ucp_reduce_scatter_nsarka_allgather_free_rkeys(coll_task);
    ucc_tl_ucp_reduce_scatter_nsarka_free_gwbi(coll_task);
...
ucc_tl_ucp_reduce_scatter_nsarka_req_test(ucs_status_ptr_t   request,
...
static inline void ucc_tl_ucp_reduce_scatter_nsarka_allgather_info_test(
    ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *     task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_service_coll_req_t *allgather_scoll_req =
        task->reduce_scatter_nsarka.allgather_scoll_req;
    ...
    ucc_tl_ucp_reduce_scatter_nsarka_allgather_info_finalize(
...
static inline void
ucc_tl_ucp_reduce_scatter_nsarka_barrier(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_base_team_t *  team = coll_task->team;
    ucc_status_t       status;
    ...
    status = ucc_tl_ucp_coll_init(&coll_args, team,
                                  &task->reduce_scatter_nsarka.barrier_task);
    if (status < 0) {
        tl_error(coll_task->team->context->lib,
...
void ucc_tl_ucp_reduce_scatter_nsarka_progress(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task    = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ...
    ucc_tl_ucp_reduce_scatter_nsarka_pipeline *pipe =
        task->reduce_scatter_nsarka.pipe;
    ucc_tl_ucp_reduce_scatter_nsarka_buf *accbuf    = &pipe->accbuf;
    ucp_request_param_t          req_param = {0};
    int                          i         = 0;
    ucc_service_coll_req_t *     allgather_scoll_req =
        task->reduce_scatter_nsarka.allgather_scoll_req;
    ucc_coll_task_t *barrier_task = task->reduce_scatter_nsarka.barrier_task;
    ...
    size_t           red_idx;
    ucc_tl_ucp_reduce_scatter_nsarka_buf *redbuf;
    ucc_tl_ucp_reduce_scatter_nsarka_buf *getbuf;
    size_t                       put_offset;
    //int                          window;
    int                          put_idx;
    ...
    if (barrier_task != NULL) {
        ...
        if (barrier_task->super.status == UCC_OK) {
            ucc_tl_ucp_put_task(
                ucc_derived_of(task->reduce_scatter_nsarka.barrier_task,
                               ucc_tl_ucp_task_t));
            task->reduce_scatter_nsarka.barrier_task = NULL;
    ...
    if (allgather_scoll_req != NULL) {
        ucc_tl_ucp_reduce_scatter_nsarka_allgather_info_test(coll_task);
        return;
    }

    if (task->reduce_scatter_nsarka.reduce_in_progress) {
        // We've previously started a reduction on the accbuf that hasn't yet
        // completed.
        ucc_tl_ucp_reduce_scatter_nsarka_test_reduction(task);
    ...
    if (pipe->count_serviced < pipe->my_count) {
        if ((pipe->count_received < pipe->my_count) &&
            ...
            src_addr =
                task->reduce_scatter_nsarka.sbufs[src_rank] + get_offset;
            dst_addr = getbuf->buf;
            ...
            getbuf->ucp_req = ucp_get_nbx(
                task->reduce_scatter_nsarka.eps[src_rank], dst_addr,
                data_size, (uint64_t)src_addr,
                task->reduce_scatter_nsarka.src_rkeys[src_rank], &req_param);
        ...
        if (accbuf->state == RECVING) {
            ...
            if (ucc_tl_ucp_reduce_scatter_nsarka_req_test(request, task) ==
                UCC_OK) {
                if (request)
        ...
        if (accbuf->state == REDUCING && redbuf->state == RECVING) {
            ...
            if (ucc_tl_ucp_reduce_scatter_nsarka_req_test(request, task) ==
                ...
                ucc_tl_ucp_reduce_scatter_nsarka_reduction(coll_task, accbuf,
                                                              redbuf);
        ...
        if ((pipe->count_serviced < pipe->count_reduced) &&
            ...
            if (pipe->posted_put == 0) {
                ...
                src_addr = accbuf->buf;
                dst_addr =
                    task->reduce_scatter_nsarka.rbufs[dst_rank] + put_offset;
                put_idx = pipe->posted_put %
                          task->reduce_scatter_nsarka.put_window_size;

                assert(task->reduce_scatter_nsarka.put_requests[put_idx] == NULL);

                ucp_worker_fence(tl_ctx->worker.ucp_worker);
                task->reduce_scatter_nsarka.put_requests[put_idx] =
                    ucp_put_nbx(
                        task->reduce_scatter_nsarka.eps[dst_rank], src_addr,
                        data_size, (uint64_t)dst_addr,
                        task->reduce_scatter_nsarka.dst_rkeys[dst_rank],
                        &req_param);
            ...
            for (i = pipe->done_put; i < pipe->posted_put; i++) {
                put_idx = i % task->reduce_scatter_nsarka.put_window_size;
                request = task->reduce_scatter_nsarka.put_requests[put_idx];

                // These are fenced, so if the first fails, the proceding will
                // too
                if (ucc_tl_ucp_reduce_scatter_nsarka_req_test(
                        request, task) != UCC_OK)
                    break;

                if (request != NULL)
                    ucp_request_free(request);
                task->reduce_scatter_nsarka.put_requests[put_idx] = NULL;
                pipe->done_put++;
            }
            ...
            if (pipe->done_put == 1) {
                ...
                ucc_tl_ucp_reduce_scatter_nsarka_reset_buf(accbuf);
                pipe->done_get = 0;
                pipe->done_red = pipe->done_put = pipe->posted_put = 0;

                for (i = 0; i < task->reduce_scatter_nsarka.put_window_size;
                     i++) {
                    task->reduce_scatter_nsarka.put_requests[i] = NULL;
                }
            }
        }
    ...
    if (pipe->count_serviced == pipe->my_count) {
        ucc_tl_ucp_reduce_scatter_nsarka_barrier(coll_task);
    }
}
