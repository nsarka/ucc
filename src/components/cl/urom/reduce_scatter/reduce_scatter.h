...
static inline int ucc_cl_urom_reduce_scatter_alg_from_str(const char *str)
{
    int i;
    for (i = 0; i < UCC_CL_UROM_REDUCE_SCATTER_ALG_LAST; i++) {
        if (0 == strcasecmp(str, ucc_cl_urom_reduce_scatter_algs[i].name)) {
            break;
        }
    }
    return i;
