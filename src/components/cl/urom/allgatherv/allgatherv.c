...
ucc_base_coll_alg_info_t
    ucc_cl_urom_allgatherv_algs[UCC_CL_UROM_ALLGATHERV_ALG_LAST + 1] = {
        [UCC_CL_UROM_ALLGATHERV_ALG_FULL] =
            {.id   = UCC_CL_UROM_ALLGATHERV_ALG_FULL,
             .name = "urom_full_offload",
             .desc = "full offload of allgatherv"},
...
ucc_status_t ucc_cl_urom_allgatherv_triggered_post_setup(ucc_coll_task_t *task)
{
    return UCC_OK;
}
...
static ucc_status_t ucc_cl_urom_allgatherv_full_start(ucc_coll_task_t *task)
{
    ...
    if (
        !(
            ...
            for (i = 0; i < UCC_CL_TEAM_SIZE(cl_team); i++) {
                ...
                uint32_t d = prev_dst_displacements[i];
    ...
    for (i = 0; i < UCC_CL_TEAM_SIZE(cl_team); i++) {
        dst_count += coll_args->dst.info_v.counts[i] * size_mod_dst;
    }

    prev_src = coll_args->src.info.mem_type;
    prev_dst = coll_args->dst.info_v.mem_type;
    ...
    buffer_export_ucc(tl_ctx->worker.ucp_context, coll_args->dst.info_v.buffer, dst_count * dt_size(coll_args->dst.info_v.datatype), dst_ebuf);
...
static ucc_status_t ucc_cl_urom_allgatherv_full_finalize(ucc_coll_task_t *task)
{
    ucc_cl_urom_schedule_t *schedule =
        ucc_derived_of(task, ucc_cl_urom_schedule_t);
    ucc_status_t status;
...
static void ucc_cl_urom_allgatherv_full_progress(ucc_coll_task_t *ctask)
{
    ...
    if (ctx->req_mc) {
        size_t size_mod = dt_size(ctask->bargs.args.dst.info_v.datatype);

        if ((ucc_status_t) notif->ucc.status == UCC_OK) {
            ucc_mc_memcpy(ctx->old_dest, ctask->bargs.args.dst.info_v.buffer, ctask->bargs.args.dst.info.count * size_mod, ctask->bargs.args.dst.info_v.mem_type, UCC_MEMORY_TYPE_HOST); // nick: fix .dst.info.count to info_v.counts
                         ...
                         ucc_coll_task_t **task)
{
    ...
    schedule->super.post           = ucc_cl_urom_allgatherv_full_start;
    schedule->super.progress       = ucc_cl_urom_allgatherv_full_progress;
    schedule->super.finalize       = ucc_cl_urom_allgatherv_full_finalize;
    schedule->super.triggered_post = ucc_triggered_post;
    schedule->super.triggered_post_setup =
        ucc_cl_urom_allgatherv_triggered_post_setup;
